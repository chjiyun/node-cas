 ...
 *     resolve([ err, auth_status, username, extended ])
 *     `extended` is an object containing:
 *       - username
 *       - attributes
 *       - PGTIOU
 ...
 * @api public
 */
CAS.prototype.validateAsync = function(ticket, service, renew) {
      ...
      res.on('end', function() {
        ...
        if (cas_version < 2.0) {
          ...
          if (sections.length >= 1) {
            if (sections[0] == 'no') {
              ...
              resolve([undefined, true, sections[1]]);
              return;
            }
          }
          // Format was not correct, error
          resolve([new Error('Bad response format.')]);
        } 
        ...
        // CAS 2.0 (XML response, and extended attributes)
          ...
          if (elemSuccess && elemSuccess.length > 0) {
              ...
              if (!elemUser || elemUser.length < 1) {
                  //  This should never happen
                  resolve([new Error("No username?"), false]);
                  return;
              }
              ...
              resolve([undefined, true, username, {
                  'username': username,
                  'attributes': attributes,
                  'PGTIOU': pgtIOU,
                  'ticket': ticket,
          ...
          if (elemFailure && elemFailure.length > 0) {
              ...
              var message = 'Validation failed [' + code +']: ';
              message += elemFailure.text();
              resolve([new Error(message), false]);
              return;
          }
